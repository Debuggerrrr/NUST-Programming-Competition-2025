...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($errors)) {
        ...
        if ($input['role'] !== 'patient') {
            $error = 'Only patients can self-register. Staff accounts must be created by administrators.';
        } else {
            $user = new User();
            if ($user->getByUsername($input['username'])) {
                ...
                $db = new Database();
                $existing_email = $db->fetch("SELECT user_id FROM users WHERE email = :email", [':email' => $input['email']]);
                if ($existing_email) {
